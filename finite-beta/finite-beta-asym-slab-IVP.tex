...
\begin{itemize}
	...
	\item Check that integral around semi-circle and vertical contours go to zero as semi-circle is extended to infinity.
	\item Calculate contributions of the contours around the branch points - corresponding to "improper modes". Including check that the integrals around the little semi-circles goes to zero.
...
\begin{cases}
\sinh(m_0(x_0 - s))\sinh(m_0(x + x_0)), & \text{if } -x_0<x<s, \\
\sinh(m_0(x_0 - x))\sinh(m_0(s + x_0)), & \text{if } s<x<x_0.
\end{cases}
\end{equation}
...
So the integrals along $C_1$ becomes
\begin{align}
...
\end{align}
where superscripts $+$ and $-$ indicate the value of the function above and below the branch cut, respectively.

For values of $\omega$ between the branch points at $\omega_0$, $\omega_1$, $\omega_2$, and $\omega_A$, the integrand is analytic, therefore we can use integration by parts in these sub-intervals. For example, the integral over the sub-interval $(\omega_0, \omega_1)$ is
\begin{align}
I_{C_1} &= \int_{\omega_0}^{\omega_1} \left( \frac{T_{1,2}^-}{D^-} - \frac{T_{1,2}^+}{D^+} \right) e^{-i\omega t} d\omega \\
...
Using the fact that $T_{1,2}^\pm(\omega)/D^\pm(\omega) \to 0$ as $|\omega| \to \infty$, we find that $I_{C_1} = \mathcal{O}(t^{-1})$ as $t \to \infty$. Similarly, $I_{C_5} = \mathcal{O}(t^{-1})$ as $t \to \infty$.
...
The residues for the slow sausage surface mode $\omega = \pm \omega_{sss}$ are 
\begin{align}
...
The residues for the fast kink body modes $\omega = \pm \omega_{fkbn}$, for $n = 1, 2, ...$ are
\begin{align}
\mathrm{Res}\left\{ \frac{T_1}{D}e^{i\omega t}; \omega = \omega_{fkbn} \right\} &= \lim_{\omega \to \omega_{fkbn}} \left\{ (\omega - \omega_{fkbn}) \frac{T_1(\omega)}{D(\omega)} e^{i\omega t} \right\} \\
&= 
\end{align}
Similarly, the residue at $\omega = -\omega_{fkbn}$ is
\begin{equation}
\mathrm{Res}\left\{ \frac{T_1}{D}e^{i\omega t}; \omega = -\omega_{fkbn} \right\} =
\end{equation}

The residues for the integrand of $A_2$ are the same but with subscript 2 instead of 1.
