...
When the above conditions hold, the zeros of the dispersion function reduce to $\omega_{0+} = kv_{A0}$ and $\omega_{0-} = kv_{AS} = k \sqrt{(v_{A1}^2 + v_{A2}^2) / 2}$ and the dispersion function itself reduces to $D(\omega) = \e_0(\e_1 + \e_2)$. Therefore, the functional reduce to
\begin{equation}
T_{1,2}(\omega) = -2\Omega_0\rho_0 \e_0,
\quad 
