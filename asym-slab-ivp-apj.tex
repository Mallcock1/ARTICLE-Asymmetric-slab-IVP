...
\end{equation}
(this differs from \cite{rae_etal81} by a factor of $-1$ due to taking Fourier forms like $e^{ikz}$ rather than $e^{-ikz}$). For utility later on, we define the function $\Psi(x, t) = k(\rho_0\hat{\Omega}(x, t) - \rho_0'\hat{v}_z(x, t))$ so that $f(x) = \omega \Psi(x, 0) + i\frac{\partial \Psi}{\partial t}(x, 0)$.
...
\subsubsection{Constant initial vorticity}
