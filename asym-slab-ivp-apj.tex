...
D(\omega) & = \epsilon_0(\epsilon_1 + \epsilon_2)\cosh(2\epsilon_0x_0) + (\epsilon_0^2 + \epsilon_1\epsilon_2)\sinh(2\epsilon_0x_0),
\label{D incomp}
\end{align}
and
\begin{equation}
...
The aim is to apply the inverse Laplace transform 
...
Since the problem of finding the solution is now reduced to solving a complex integral, it is dependent on the singularities (with respect to $\omega$) of $T_1$, $T_2$, and $D$ (so that we can construct the Bromwich contour such that it doesn't enclose any singularities) and the zeros of $D$ (whose residues determine the value of the contour integral).
